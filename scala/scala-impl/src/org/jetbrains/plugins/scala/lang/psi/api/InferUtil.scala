      ...
      undefSubst.substitutionBounds(canThrowSCE) match {
        case Some(bounds@SubstitutionBounds(_, lowerMap, upperMap)) =>
          ...
          if (!filterTypeParams) {
            ...
            val newUndefined = typeParams.foldLeft(undefSubst)(addConstraints)

            def updateWithSubst(sub: ScSubstitutor) = ScTypePolymorphicType(
              sub.subst(retType),
              typeParams.filter { tp =>
                val removeMe = newUndefined.isApplicable(tp.typeParamId)
            ...
            newUndefined match {
              case ScUndefinedSubstitutor(substitutor) => updateWithSubst(substitutor)
